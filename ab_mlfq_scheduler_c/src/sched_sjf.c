...
static void record_timeline(TimelineEntry timeline[], int *count, int time, int pid) {
    ...
    strcpy(timeline[*count].algorithm, "sjf");
...
int run_sjf(Process src[], int n, const char *scenario, Metrics *metrics) {
    ...
    while (completed < n) {
        if (running_idx < 0) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (processes[i].remaining_time < best_burst ||
                    (processes[i].remaining_time == best_burst &&
                     processes[i].arrival_time < best_arrival)) {
                    best = i;
                    best_burst = processes[i].remaining_time;
    ...
    compute_metrics(metrics, "sjf", scenario, processes, n, context_switches, current_time, busy_time);
    write_process_stats_csv("sjf", scenario, processes, n);
    write_timeline_csv("sjf", scenario, timeline, timeline_count);
    free(timeline);
    return 0;
}
